...
## This script made by @Excerion
# https://github.com/Excerion/dswitcher
...
num=$(wmctrl -l | sed 's/  / /' | cut -d " " -f 4- | nl -w 3 -n rn | sed -r 's/^([ 0-9]+)[ \t]*(.*)$/\1 - \2/' | dmenu -i -p "Window" -l $heightfit | cut -d '-' -f -1)
[[ -z "$num" ]] && exit
